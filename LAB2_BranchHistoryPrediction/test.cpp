...
{ 	
	...
	std::thread t1(threadFunc, BHR, PHT_counter);
    std::thread t2(threadFunc, BHR, PHT_counter);
    std::thread t3(threadFunc, BHR, PHT_counter);
...
void threadFunc(vector<unsigned int> BHR, vector<unsigned long> PHT_counter) {
	 ifstream trace;
	 ofstream traceout;
